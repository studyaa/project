...
def getInfoList(url):
    ...
    info_list=html.xpath('//div[@class="slider-item"]')
    return info_list

def fillList(info,infos):
    infos['title']=info.xpath('./a/span/text()')[0]
    ...
    infos['url']=detail_url
    resp_detail = requests.get(url=detail_url)
...
def printInfo(infos,info_list):
    ...
    for info in info_list:
        ...
        sqla = '''
                insert into sina(title,img,url,html)
                values(%s,%s,%s,%s);
               '''
        try:
